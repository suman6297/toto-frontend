# Toto Website Contributing Guide
...
1. Clone this repo to your local machine with one of the commands below.
    ...
    git clone git@github.com:Dev-Sisyphus/Toto-Website-Frontend.git
    # Otherwise for HTTPS:
    git clone https://github.com/Dev-Sisyphus/Toto-Website-Frontend.git
...
3. Sync your work with the origin remote every so often. replacing the `<your branch name>` with the branch you've been working on locally:
   ...
   ```
   ### N.B: 
   Instead of git fetch and git merge, you can also use git pull. The main difference is that `git fetch` does not automatically merges the changes into the current branch whereas `git pull` does. So if you use `git fetch` then you have to use `git merge` to reflect the changes. `git pull` does these two works all alone and at a time. 

   <img src="public/image.png" style="width: 300px; height: 150px">
